    ...
    if interface == False:
        ...
        if group == 0:
            for i in range(double_use_bit_len):
                # random a loc to minus
                loc = np.random.randint(0, len(challenges[0]))
                # delete at loc position
                challenges = np.delete(challenges, loc, axis=1)
        else:
            # set up consecutive bits length
            # group_len = int(double_use_bit_len / group)
            group_len = random_num_with_fix_total(double_use_bit_len, group)
            for i in range(group):
                # random a loc to minus
                loc = np.random.randint(0, len(challenges[0] - max(group_len)))
                for j in range(group_len[i]):
                    # check boundry
                    if loc >= len(challenges[0]):
                        loc = len(challenges[0]) - 1
...
def run(n: int, k: int, N: int, seed_sim: int, noisiness: float, BATCH_SIZE: int, interface: bool,
    ...
    epochs = 200
    print('hello')
    challenges, responses = initialize_and_tranform_PUF(n, k, N, seed_sim, noisiness, interface, double_use_bit_len,
                                                        group, puf)
    ...
    model.add(
        layers.Dense(pow(2, k) / 2, activation='tanh', input_dim=n - double_use_bit_len,
    ...
    if puf == "ffpuf":
        ...
        if interface == 1:
            with open('ffpuf_interface_minus.csv', 'a') as f:
                # using csv.writer method from CSV package
                write = csv.writer(f)
                if seed_sim == 0:
                    write.writerow(fields)
        ...
        if k == 1:
            if interface == 0:
                with open('1_64xpuf_without_interface.csv', 'a') as f:
                    ...
                    write.writerows(rows)
            if interface == 1:
                with open('1_64xpuf_interface_minus.csv', 'a') as f:
                    # using csv.writer method from CSV package
                    write = csv.writer(f)
                    if seed_sim == 0:
                        write.writerow(fields)
        ...
        if k == 3:
            if interface == 0:
                with open('3_64xpuf_without_interface.csv', 'a') as f:
                    ...
                    write.writerows(rows)
            if interface == 1:
                with open('3_64xpuf_interface_minus.csv', 'a') as f:
                    # using csv.writer method from CSV package
                    write = csv.writer(f)
                    if seed_sim == 0:
                        write.writerow(fields)
...
def main(argv=None):
    ...
    interface = True
    # puf = "ffpuf"
    # run(64, 1, 10000000, 0, 0.00, 100000, interface, 6, 0,puf)
    # APUF
    double_use_bit_len1 = [4, 6, 8, 10, 12]
    puf = "apuf"
    for i in seed:
